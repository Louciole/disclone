...
class Mycelium(ForumMixin, Server):
    ...
    def _mobility_request_json(self, url, params, config):
        ...
        self._mobility_apply_key(params, headers, config)
    ...
    def transport_lines(self, operator):
        self.getUser()
        config, op_cfg = self._mobility_get_operator(operator)
        ...
        params = self._mobility_build_params(lines_cfg.get("params"), {"operator": operator})
        payload = self._mobility_request_json(lines_cfg["url"], params, config)
        items = self._mobility_extract_items(payload, lines_cfg.get("items_path"))

        id_field = lines_cfg.get("id_field", "id")
        label_field = lines_cfg.get("label_field", "name")
        lines = []
        for item in items:
            fields = self._mobility_extract_fields(item)
            line_id = self._mobility_get_field(fields, id_field)
            if line_id is None:
                continue
            label = self._mobility_get_field(fields, label_field)
    ...
    def transport_stops(self, operator, line=None):
        ...
        params = self._mobility_build_params(stops_cfg.get("params"), {"operator": operator, "line": line or ""})
        payload = self._mobility_request_json(stops_cfg["url"], params, config)
        items = self._mobility_extract_items(payload, stops_cfg.get("items_path"))
        ...
        for item in items:
            fields = self._mobility_extract_fields(item)
            stop_id = self._mobility_get_field(fields, id_field)
            if stop_id is None:
                continue
            label = self._mobility_get_field(fields, label_field)
    ...
    def transport_arrivals(self, operator, stop=None, line=None):
        ...
        params = self._mobility_build_params(
            arrivals_cfg.get("params"),
            {"operator": operator, "line": line or "", "stop": stop}
        )
        payload = self._mobility_request_json(arrivals_cfg["url"], params, config)
        items = self._mobility_extract_items(payload, arrivals_cfg.get("items_path"))
        ...
        for item in items:
            fields = self._mobility_extract_fields(item)
            arrival_time = self._mobility_get_field(fields, time_field)
            minutes = self._mobility_minutes_from_time(arrival_time)
            arrivals.append({
                "arrival_time": arrival_time,
                "minutes": minutes,
                "destination": self._mobility_get_field(fields, destination_field),
                "line": self._mobility_get_field(fields, line_field)
            })
