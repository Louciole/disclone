...
window.factionsDescriptions = {
    solo: "Le Solo est un personnage énigmatique qui évolue dans les marges. Son alignement moral se traduit par un code d'honneur personnel qu'il suit scrupuleusement. Il n'est pas redevable aux corporations ou à la loi officielle, mais à ses propres principes.\n" +
    "\n" +
    ...
    nomade: "Le Nomade est un esprit indépendant et farouche qui sillonne les vastes étendues du désert. Son alignement chaotique le pousse à rejeter les structures et les contrôles imposés par les corporations et les cités. Pour lui, la liberté prime sur tout.\n" +
        "\n" +
    ...
    netrunner: "Le Netrunner est un esprit libre qui navigue les vastes étendues du Net. Son alignement chaotique le pousse à défier les puissances et les systèmes d'exploitation qui contrôlent la circulation de l'information.\n" +
        "\n" +
    ...
    fixeur: "Le Fixeur est le rouage discret qui fait tourner la machine chaotique de notre univers. Son alignement moral le pousse à maintenir un certain ordre social dans ce monde impitoyable. Il comprend que sans un minimum de structure et de confiance, la ville sombrerait complètement dans l'anarchie.\n" +
        "\n" +
...
const FACTION_SVG = {solo:'solo', nomade:'nomades', netrunner:'netrunner', fixeur:'fixeurs', psycho:'psychos', corpo:'corpo'}

function getFactionBadgeHtml(name) {
    const file = FACTION_SVG[name]
    return file ? `<img class="badge" src="/static/images/${file}.svg" title="${name}">` : ''
}
window.getFactionBadgeHtml = getFactionBadgeHtml
...
function finishQuizzz(){
    ...
    const result = getQuizResult(answerCount)
    setElement("global.user.faction", result)
    xhr("change_profile?element=faction&value=" + encodeURIComponent(result), undefined, "POST", false)
